...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen font-sans bg-gray-50 selection:bg-orange-100 selection:text-orange-900 overflow-x-hidden">
      <header className="fixed w-full top-0 z-50 transition-all duration-300 backdrop-blur-md bg-[#1A225F]/95 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center select-none cursor-pointer" onClick={() => window.scrollTo(0,0)}>
            <span className="text-white text-2xl font-black tracking-wide">PROCUR</span>
            <span className="text-2xl font-black tracking-wide relative" style={{ color: colors.secondary }}>
              AÍ
              <span className="absolute -top-1 right-[1px] text-sm">´</span>
            </span>
          </div>
          <div className="flex items-center gap-4">
            <button 
              onClick={() => navigate('/login')}
              className="text-white font-semibold hover:text-orange-200 transition text-sm md:text-base"
            >
              Entrar
            </button>
            <button 
              onClick={() => navigate('/cadastro')}
              className="text-white px-6 py-2.5 rounded-full font-bold text-sm hover:brightness-110 transition shadow-lg"
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="text-center lg:text-left space-y-8 animate-in slide-in-from-left duration-700">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4">
              <button 
                className="text-white text-lg font-bold px-10 py-4 rounded-xl transition shadow-xl hover:shadow-orange-500/20 hover:-translate-y-1 transform flex items-center justify-center gap-2 w-full sm:w-auto"
                style={{ backgroundColor: colors.secondary }}
                onClick={() => navigate('/cadastro')}
              >
                Começar agora
